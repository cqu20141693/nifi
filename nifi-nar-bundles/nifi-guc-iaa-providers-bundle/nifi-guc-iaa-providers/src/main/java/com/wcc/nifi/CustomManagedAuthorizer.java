 ...
 * wcc 2022/10/25
 */
public class CustomManagedAuthorizer implements ManagedAuthorizer {
    private static final XMLOutputFactory XML_OUTPUT_FACTORY = XMLOutputFactory.newInstance();

    private static final Logger LOGGER = LoggerFactory.getLogger(CustomManagedAuthorizer.class);
    ...
    private final Set<String> allowAllUser = new HashSet<>();

    private final Set<String> adminUser = new HashSet<>();

    @Override
    public void initialize(AuthorizerInitializationContext initializationContext) throws AuthorizerCreationException {
        accessPolicyProviderLookup = initializationContext.getAccessPolicyProviderLookup();
        String[] resources = {"/flow", "/counters", "/provenance", "/tenants", "/controller", "/policies", "/system", "/parameter-contexts",
                "/restricted-components/access-keytab", "/restricted-components/access-ticket-cache",
                "/restricted-components", "/restricted-components/read-filesystem", "/restricted-components/write-filesystem",
                "/restricted-components/read-distributed-filesystem", "/restricted-components/write-distributed-filesystem",
                "/restricted-components/execute-code", "/restricted-components/access-environment-credentials",
                "/restricted-components/export-nifi-details"};
        for (String resource : resources) {
            allowAllUser.add(resource + delimiter + RequestAction.READ.name());
            allowAllUser.add(resource + delimiter + RequestAction.WRITE.name());
        }
        adminUser.add("admin");
    }
    ...
    private boolean preCheck(AuthorizationRequest request) {
        String resource = String.join(delimiter, request.getResource().getIdentifier(), request.getAction().name());
        LOGGER.info("authorizer resource {}", resource);
        return adminUser.contains(request.getIdentity()) || allowAllUser.contains(resource);
    }
